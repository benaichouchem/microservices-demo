...
import (
	...
	cb "github.com/benaichouchem/microservices-demo/src/checkoutservice/genproto"
)
...
func IsValid(m cb.Money) bool {
	return signMatches(m) && validNanos(m.GetNanos())
}

func signMatches(m cb.Money) bool {
...
func IsZero(m cb.Money) bool { return m.GetUnits() == 0 && m.GetNanos() == 0 }

// IsPositive returns true if the specified money value is valid and is
// positive.
func IsPositive(m cb.Money) bool {
...
func AreEquals(l, r cb.Money) bool {
...
func Negate(m cb.Money) cb.Money {
	return cb.Money{
		Units:        -m.GetUnits(),
		Nanos:        -m.GetNanos(),
...
func Must(v cb.Money, err error) cb.Money {
	if err != nil {
		panic(err)
	}
	return v
...
// both).
func Sum(l, r cb.Money) (cb.Money, error) {
	if !IsValid(l) || !IsValid(r) {
		return cb.Money{}, ErrInvalidValue
	} else if l.GetCurrencyCode() != r.GetCurrencyCode() {
		return cb.Money{}, ErrMismatchingCurrency
	}
...
// to itself n-1 times.
func MultiplySlow(m cb.Money, n uint32) cb.Money {
	out := m
	for n > 1 {
		out = Must(Sum(out, m))
		n--
